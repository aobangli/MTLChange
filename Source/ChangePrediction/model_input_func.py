from Source.ChangePrediction.TrainConfig import num_of_dense_feature
...
def predict_by_aitm(x, model):
    dense_input, sparse_inputs = x[:, :num_of_dense_feature], x[:, num_of_dense_feature:]
    outputs = model(sparse_inputs.long(), dense_input)
    return outputs

def predict_by_snr_trans(x, model):
    return model(x)
...
func_dict = {
    ...
    'aitm': predict_by_aitm,
    'snr_trans': predict_by_snr_trans
}
