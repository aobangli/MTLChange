import torch
import numpy as np
from Source.ChangePrediction.models.layers import EmbeddingLayer, MultiLayerPerceptron
...
class AITMModel(torch.nn.Module):
    ...
    def __init__(self, categorical_field_dims, numerical_num, embed_dim, bottom_mlp_dims, tower_mlp_dims, task_num,
                 task_types, task_out_dims, dropout, device):
        super().__init__()
        ...
        self.g = torch.nn.ModuleList(
            [torch.nn.Linear(bottom_mlp_dims[-1], bottom_mlp_dims[-1]) for i in range(task_num - 1)])
        ...
        self.tower = torch.nn.ModuleList(
            [MultiLayerPerceptron(bottom_mlp_dims[-1], tower_mlp_dims, dropout, task_type=task_types[i], task_out_dim=task_out_dims[i]) for i in
             range(task_num)])
    ...
    def forward(self, categorical_x, numerical_x):
        ...
        for i in range(self.task_num):
            output = self.tower[i](fea[i])
            task_outs.append(output)
        results = torch.cat(task_outs, -1)
        return results
