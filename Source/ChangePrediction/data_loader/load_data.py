...
def resample(df, label):
    columns_except_label = list(df.columns)
    columns_except_label.remove(label)

    x_data = df[columns_except_label]
    y_data = df[[label]]
...
def prepare_dataframe():
    ...
    # print(f"time <= {low} : {df[df['time'] <= low].shape[0]}")
    # print(f"time >= {high} : {df[df['time'] >= high].shape[0]}")
    # df = df[(df['time'] > low) & (df['time'] < high)]
    # print(f"left : {df.shape[0]}")

    if end_date is not None and end_date != "":
        df['created'] = pd.to_datetime(df['created'])
        e_date = datetime.datetime.strptime(end_date, '%Y-%m-%d').date()
        df = df[df['created'].dt.date < e_date]
...
def load_splited_dataframe():
    ...
    # train_df = resample(train_df, 'avg_score')
