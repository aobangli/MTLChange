...
from Source.ChangePrediction.model_input_func import func_dict
...
class MultiTrainerWeightingLoss:
    ...
    def train(self, train_dataset):
        ...
        # 使用tensorboard画loss图
        writer = SummaryWriter(f'../tensorboard_logs/{self.config["model_name"]}_{time_str}')
        # loss的横坐标
        loss_index = 0
        # weight的横坐标
        weight_index = 0
        ...
        for epoch in range(self.config['num_epoch']):
            ...
            for batch, train_data in enumerate(train_loader):
                ...
                all_labels_loss = torch.zeros(num_of_labels).to(self.device)
                ...
                for label_index, label in enumerate(target_labels):
                    ...
                    if print_batch_log and batch % 10 == 0:
